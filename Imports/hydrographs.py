...
def show_dem_change_plot(change_mat, cell_size, date_range):
    ...
    plt.title("Soil Erosion vs Deposit (" + str(date_range[0]) + "-" + str(date_range[1]) + ")")
    plt.legend()
    plt.show()
...
def show_full_plot(rainfall_dict, runoff_dict, date_range):
    ...
    plt.xlabel("Day")
    plt.ylabel("Rainfall, Runoff (mm)")
    plt.title("Total Rainfall vs Runoff (" + str(date_range[0]) + "-" + str(date_range[1]) + ")")
    plt.legend()
    plt.show()
    ...
    plt.xlabel("Rainfall (mm)")
    plt.ylabel("Runoff (mm)")
    plt.title("Rainfall-Runoff Regression Fit (" + str(date_range[0]) + "-" + str(date_range[1]) + ")")
    plt.legend()
    plt.show()
...
def get_date_range(rainfall_dict):
    years = rainfall_dict.keys()
    return min(years), max(years)

def generate_results(weather_file, runoff_file, input_dem, eroded_dem, year_list, cell_size, output_dir):
    ...
    minyr, maxyr = get_date_range(rainfall_dict)
    show_full_plot(rainfall_dict, runoff_dict, (minyr, maxyr))
    show_yearwise_plot(rainfall_dict, runoff_dict, year_list)
    change_mat = Mat.get_change_mat(original_dem_mat, eroded_dem_mat)
    show_dem_change_plot(change_mat, cell_size, (minyr, maxyr))
    Mat.generate_change_raster(change_mat).save(output_dir + "/simulated_dem_change.png")